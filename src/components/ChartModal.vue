  ...
  <div v-if="modelValue" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg shadow-lg w-[500px]">
      ...
      <div class="p-6">
        ...
        <select v-model="sortType" @change="fetchRates(sortType)" class="w-full p-2 border rounded-lg">
          <option v-for="item in sortItems" :key="item.value" :value="item.value">{{ item.text }}</option>
        </select>
